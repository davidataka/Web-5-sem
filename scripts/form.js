...
window.addEventListener("load", () => {
    ...
    check_buttons.forEach(function (item, i) {
        item.addEventListener("click", () => {
            checkPlayer(players_copy[i].name, players_copy[i].status);
            item.parentElement.setAttribute("player_option", players_copy[i].status);
        });
    });
});
